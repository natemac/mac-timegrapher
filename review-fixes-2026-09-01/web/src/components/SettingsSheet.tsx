...
export function parseDrift(text: string): number | null {
  const normalized = text
    ...
    .replace(/\s+s\/?d(?:ay)?$/i, '')
    .trim();
  // Number.parseFloat accepts a numeric prefix and ignores the rest. A clock
  // correction silently changes every rate reading, so partial input such as
  // "1.7oops" or "1..7" must be rejected rather than applied as 1.7 or 1.
