...
export default function App() {
  ...
  */
  ...
  /*
    Identifies the start request that is still allowed to publish a session.

    Returning to the opening screen can happen while getUserMedia and the audio
    graph are still being built. `capturing` is not true until that work ends,
    so it cannot by itself tell goHome() that there is a microphone request to
    cancel. Advancing this generation makes the eventual session tear itself
    down before it can become visible state.
  */
  ...
  const start = async () => {
    ...
    try {
      ...
      if (attempt !== captureAttempt.current) {
        // The operator returned home while the browser was opening the input.
        // This session was never published, so it has to clean itself up here.
        await s.stop().catch(() => {});
        return;
      }
      session.current = s;
      ...
      // A cancelled request belongs to the screen that was left, and must not
      // put its late error onto the opening screen.
  ...
  */
  const goHome = async () => {
    // Cancel an input that is still opening. If it has already published its
    // session but React has not painted `capturing` yet, session.current catches
    // that narrow gap and stop() still releases it.
